...
4. **재현 챌린지(미션)** — 실제 사례를 재현하도록 조작하고 모델 기반 유사도로 채점·뱃지 (체험·참여형)
5. **기후변화 시나리오** — 현재/+1.5°C/+3.0°C 토글로 같은 전선이 미래에 얼마나 강해지는지 체감 (기후 연계)
6. **AI 교육 피드백 + XAI** — 서러게이트 출력 기반 해석 + 요인 분해 + 선택적 Gemini/Grok 연동
7. **5개 프리셋 시나리오** — 실제 과거 사례 기반
8. **반응형** — 모바일에서도 완벽 동작

자세한 기획·심사기준 매핑은 [`PROPOSAL.md`](./PROPOSAL.md) 참고.
...
├── ClimateScenario.tsx     # 기후변화 시나리오 모드 (기후 연계)
├── ChallengeMode.tsx       # 재현 챌린지(미션) + 모델 기반 채점
